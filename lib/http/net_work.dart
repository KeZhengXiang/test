...
class NetWork {
  ...
  void getHttp() async {
    try {
      Response response = await dio.get("http://c.m.163.com/nc/article/headline/T1348647853363/0-40.html");
      if (response.statusCode != Code.SUCCESS){
        return;
      }
//      print(response.data["T1348647853363"]);
      List<HeadLineModel> headLinesModel = [];
      (response.data["T1348647853363"] as List).forEach((val) => headLinesModel.add(HeadLineModel.fromJson(val)));
//      print(headLinesModel);
    } catch (e) {
      print(e);
    }
  }
...
class HttpUtils {
  ...
  static Future<Map> request(String url, {data, method}) async {
    ...
    print("请求地址：【$method $url】");//LogUtils
    print("请求参数：【$data】");
